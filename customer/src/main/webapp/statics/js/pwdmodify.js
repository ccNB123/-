...
$(function() {
	...
	oldpassword.on("blur",function(){
		$.ajax({
			...
			success:function(data){
				if(data){//旧密码正确
					validateTip(oldpassword.next(),{"color":"green"},"√",true);
				}else if(data == false){//旧密码输入不正确
					validateTip(oldpassword.next(),{"color":"red"}," 原密码输入不正确",false);
	...
	rnewpassword.on("focus",function(){
		...
		if(rnewpassword.val() != null 
				&& rnewpassword.val().length < 20 && newpassword.val() == rnewpassword.val()){
			validateTip(rnewpassword.next(),{"color":"green"},"√",true);
		}else{
			validateTip(rnewpassword.next(),{"color":"red"}," 两次密码输入不一致，请重新输入",false);
		}
	});
